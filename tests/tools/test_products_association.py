...
from qontak_mcp.tools.products_association import register_products_association_tools_lazy
...
class TestProductsAssociationTools:
    @pytest.mark.asyncio
    async def test_tools_registered(self, mock_mcp, mock_client_factory):
        """Test that products_association tools are registered."""
    ...
    @pytest.mark.asyncio
    async def test_list_products_associations(self, mock_mcp, mock_client_factory, client):
        """Test list_products_associations tool."""
        register_products_association_tools_lazy(mock_mcp, mock_client_factory)
        tool = mock_mcp.tools["list_products_associations"]
        
        with patch.object(client, 'list_products_associations', return_value={"success": True, "data": {"data": []}}):
